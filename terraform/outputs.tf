...
output "aws_iam_role_hamzaelaouane_eks_cluster" {
  description = "AWS IAM role for hamzaelaouane-eks-cluster eks cluster"
  value       = aws_iam_role.hamzaVotingApp-eks-cluster.id
}

output "aws_iam_role_hamzaelaouane_eks_cluster_node_group" {
  description = "AWS IAM role for diu-eks-cluster-node-group eks cluster"
  value       = aws_iam_role.hamzaVotingApp-eks-cluster-node-group.id
}
# Uncomment  for "iam.tf" lecture - end


# Uncomment  for "sg.tf" lecture - start
output "aws_sg_hamzaelaouane_eks_cluster_node_group" {
  description = "AWS Security Group for hamzaelaouane-eks-cluster-node-group eks cluster"
...
output "aws_subnet_list_of_ids" {
  ...
  value       = [for subnet in [for value in aws_subnet.voting_app_subnet: value] : subnet.id]
}
...
output "endpoint" {
  ...
  value       = aws_eks_cluster.voting_app_eks_cluster.endpoint
}

output "kubeconfig-certificate-authority-data" {
  description = "AWS EKS cluster (Kubernetes control plane) kubeconfig"
  value       = aws_eks_cluster.voting_app_eks_cluster.certificate_authority.0.data
}
...
output "aws_eks_node_group" {
  ...
  value       = aws_eks_node_group.hamzaVotingApp_node.id
}
# Uncomment  for "main.tf" aws_eks_cluster resource lecture - start
